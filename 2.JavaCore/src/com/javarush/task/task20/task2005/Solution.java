    ...
    public static class Human {
        ...
        public void save(OutputStream outputStream) throws Exception {
            ...
            if (this.assets.size() > 0) {
                for (Asset a : this.assets) {
                    printWriter.println(this.name + "@@@" + a.getName() + "@@@" + a.getPrice());
                    printWriter.flush();
                }
            } else printWriter.println(this.name + "@@@");
            printWriter.close();
        }
        ...
        public void load(InputStream inputStream) throws Exception {
            ...
            while (bufferedReader.ready()) {
                ...
                human_name = arguments[0];
                if (arguments.length > 1) {
                    String asset_name = arguments[1];
                    Asset a = new Asset(asset_name);
                    if (arguments.length > 2) {
                        double asset_price = Double.parseDouble(arguments[2]);
                        a.setPrice(asset_price);
                    }
                    assets.add(a);
                }
            }
